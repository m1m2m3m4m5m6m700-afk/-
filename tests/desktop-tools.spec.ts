...
test.describe("relaunched public tools", () => {
  ...
  test.describe("QR Generator output correctness", () => {
    ...
    test("URL payload generates valid PNG and SVG artifacts", async ({ page }) => {
    ...
    test("plain Unicode text generates valid PNG and SVG artifacts", async ({ page }) => {
      await page.locator('button[data-qr-mode-button="text"]').click();
    ...
    test("Wi-Fi payload preserves required escaping exactly", async ({ page }) => {
      await page.locator('button[data-qr-mode-button="wifi"]').click();
    ...
    test("email payload preserves address and encoded subject", async ({ page }) => {
      await page.locator('button[data-qr-mode-button="email"]').click();
    ...
    test("phone payload generates a valid QR artifact", async ({ page }) => {
      await page.locator('button[data-qr-mode-button="phone"]').click();
      const expected = "tel:+201001234567";
      await page.locator('[data-qr-input="phone"]').fill("+201001234567");
      await assertQrDownloads(page, expected);
    });

    test("long Unicode text generates valid artifacts without output corruption", async ({ page }) => {
      await page.locator('button[data-qr-mode-button="text"]').click();
