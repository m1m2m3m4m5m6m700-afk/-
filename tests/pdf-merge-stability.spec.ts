...
async function certify(page: Page) {
  ...
  expect(Math.round(merged.getPage(0).getWidth())).toBe(300);
  expect(Math.round(merged.getPage(1).getWidth())).toBe(310);
  expect(Math.round(merged.getPage(2).getWidth())).toBe(300);
}

test.describe("PDF Merge stability", () => {
  test("merge output remains correct across independent browser runs", async ({ page }) => {
    await certify(page);
  });
});
