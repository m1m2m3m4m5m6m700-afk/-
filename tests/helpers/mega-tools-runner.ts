...
const EXPECTED_VARIANT_COUNT = 526;
...
function validateCatalog(): void {
  if (MEGA_TOOLS.length !== EXPECTED_VARIANT_COUNT) {
    throw new Error(
      `Mega-tool catalog contract mismatch: expected ${EXPECTED_VARIANT_COUNT}, found ${MEGA_TOOLS.length}. ` +
        "Update the catalog contract only when a variant is intentionally added or removed.",
    );
  }
}

function validateInput(variant: MegaVariant): void {
  if (!variant || !variant.slug || !variant.category || !variant.handler || !variant.preset) {
...
export async function prepareMegaToolPage(page: Page): Promise<void> {
  ...
  pdfPage.drawText("Flixo operational test", {
    x: 30,
    y: 240,
    size: 16,
    font,
  ...
  await page.evaluate(async (pdfBase64Value) => {
    ...
    if (!MediaRecorder.isTypeSupported("video/webm")) {
      throw new Error("Chromium does not support video/webm MediaRecorder fixtures.");
    }
...
export async function runMegaToolVariant(
  page: Page,
  variant: MegaVariant,
): Promise<MegaRunResult> {
  ...
  try {
        ...
        const run = async () => {
          ...
          const fixtures = (
            window as unknown as {
              __flixoMegaFixtures: Record<string, File>;
            }
          ).__flixoMegaFixtures;

          const fixture = fixtures[definition.category];
          if (!fixture) {
            throw new Error(`Missing fixture for ${definition.category}`);
          }
          ...
          if (result.type === "download") {
            if (!result.filename || !result.url) {
              throw new Error("Tool returned an invalid download result.");
            }
          ...
          if (result.type === "video") {
            if (result.element.tagName !== "VIDEO") {
              throw new Error("Tool returned an invalid video result.");
            }
            result.cleanup();
            return { success: true, type: result.type };
          }
        ...
        try {
          return await Promise.race([
            ...
            new Promise<never>((_, reject) => {
              timer = setTimeout(() => {
                reject(new Error(`Variant timed out after ${timeoutMs}ms.`));
              }, timeoutMs);
            }),
          ]);
        } finally {
    ...
    console.error(
      `[mega-tool] FAIL ${label} (${Date.now() - startedAt}ms): ${reason}`,
    );
