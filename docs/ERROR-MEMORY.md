...
- Every incident must state a durable prevention mechanism and evidence.
...
| `SITEMAP_XML_QUERY_FALSE_POSITIVE` | The sitemap validator scanned the XML declaration instead of only `<loc>` URL values | Parse/validate `<loc>` entries only; ignore XML declaration syntax | SEO/test | PR #126 SEO validation failure |
| `DUPLICATE_LOCALIZED_ROUTE` | Generic localized routes duplicated the dedicated EN/AR compressor routes | Exclude special routes from the generic matrix and assert the final route count | architecture | PR #126 route-matrix E2E failure |
| `UNAVAILABLE_TOOL_200` | SPA fallback exposed an unavailable tool as HTTP 200 | Return HTTP 404 and attach `noindex,follow` for unavailable/missing tools | SEO/runtime | PR #126 crawlability E2E failure |
...
| `HARD_CODED_TOOL_UI_TEXT` | User-visible JSX bypassed locale dictionaries | Typed tool UI dictionaries plus AST `i18n:check` gate | i18n | PR #126 i18n gate failure |
| `VERCEL_BUILD_RATE_LIMIT` | Vercel Hobby deployment capacity was exhausted during repeated preview builds | Treat this as external capacity; avoid redeploy loops and do not classify it as an application-code failure | external | PR #126 Vercel deployment failures |
...
|---|---|---|---|
| `CANONICAL_ORIGIN_UNAVAILABLE` | No final HTTPS production domain is configured; only temporary Vercel domains exist | Refuse production canonical/sitemap output until `VITE_SITE_URL` is a real HTTPS origin | PR #126 Vercel project state |
...
|---|---|---|
| `INEFFECTIVE_LOCALE_DYNAMIC_IMPORT` | Keep compile-time locale typing separate from runtime locale chunks | PR #126 locale-loader review |

## Error-memory contract

A memory entry is actionable only when it contains a unique signature, verified root cause, durable prevention, class, and evidence. The validator enforces uniqueness and structure, but the presence of a record does not prove that the underlying incident can never recur.
