  ...
  return (
    <section className="mx-auto w-full max-w-5xl" aria-label="Flex AI chat">
      <div className="overflow-hidden rounded-[2rem] border border-border/70 bg-card/80 shadow-lift backdrop-blur-xl">
              ...
              <div className="mt-5 flex max-w-3xl flex-wrap justify-center gap-2">
                {suggestions.map((suggestion) => (
                  <button
                    ...
                    className="rounded-full border border-border/70 bg-background px-3 py-2 text-xs text-muted-foreground hover:border-primary/40 hover:text-foreground"
                  >
                    {suggestion}
                  </button>
                ))}
        ...
        {error && (
          <div className="mx-4 mb-3 rounded-xl border border-destructive/20 bg-destructive/5 px-3 py-2 text-xs text-destructive sm:mx-6">
            {error}
          </div>
        )}
          ...
          <div className="rounded-2xl border border-border/80 bg-card p-2 focus-within:border-primary/40 focus-within:ring-2 focus-within:ring-primary/10">
            ...
            <div className="flex items-center justify-between gap-2 px-1 pt-1">
              <div className="flex items-center gap-2 text-[11px] text-muted-foreground">
                ...
                <button type="button" onClick={newChat} className="hover:text-foreground">
                  {t("assistant.reset")}
                </button>
              </div>
              <Button type="submit" disabled={!input.trim() || loading} className="rounded-xl px-4 font-bold">
