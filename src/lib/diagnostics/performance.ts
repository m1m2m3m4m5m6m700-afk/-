...
function save(entry: CoreWebVitalsSnapshot | ToolPerformanceMetric): void {
  try {
    const raw = localStorage.getItem(STORAGE_KEY);
    const current = raw ? (JSON.parse(raw) as Array<CoreWebVitalsSnapshot | ToolPerformanceMetric>) : [];
    current.push(entry);
...
export function getPerformanceDiagnostics(): Array<CoreWebVitalsSnapshot | ToolPerformanceMetric> {
  try {
    const raw = localStorage.getItem(STORAGE_KEY);
    return raw ? (JSON.parse(raw) as Array<CoreWebVitalsSnapshot | ToolPerformanceMetric>) : [];
  } catch {
    return [];
  }
}
