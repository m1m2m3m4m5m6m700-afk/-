...
const surveyQuestionTypeSchema = z.enum(["single_choice", "multi_choice", "dropdown", "scale", "rating", "nps", "yes_no", "text", "textarea", "number", "date", "email", "url", "ranking", "matrix_single", "matrix_multi", "consent"]);
const serializableConfigValueSchema = z.union([z.string(), z.number(), z.boolean(), z.null(), z.array(z.string())]);
const questionConfigSchema = z.record(z.string(), serializableConfigValueSchema).default({});
...
export const getAdminBehaviorOverview = createServerFn({ method: "GET" }).middleware([adminSessionMiddleware]).validator(z.object({ days: daysSchema })).handler(async ({ context, data }) => {
  ...
  return { ok: true as const, periodDays: data.days, sessions: sessionIds.size, sessionStarts: rows.filter((row) => row.eventType === "session_start").length, pageViews: rows.filter((row) => row.eventType === "page_view").length, searches: rows.filter((row) => row.eventType === "search").length, toolStarts: rows.filter((row) => row.eventType === "tool_start").length, toolCompletions: rows.filter((row) => row.eventType === "tool_complete").length, surveyResponses: rows.filter((row) => row.eventType === "survey_response").length, clickEvents: rows.filter((row) => ["tool_click", "category_click", "external_link_click", "download", "copy"].includes(row.eventType)).length, averageJourneyMs: durations.length ? Math.round(durations.reduce((sum, value) => sum + value, 0) / durations.length) : 0, topTools: top(rows.map((row) => row.toolId)), topIntents: top(rows.map((row) => row.intentId)), topPages: top(rows.map((row) => row.path)), pathTransitions: top(rows.filter((row) => row.previousPath && row.path).map((row) => `${row.previousPath} → ${row.path}`)), locales: top(rows.map((row) => row.locale)), eventMix: top(rows.map((row) => row.eventType), 20) };
});

export const getAdminSurveys = createServerFn({ method: "GET" }).middleware([adminSessionMiddleware]).handler(async ({ context }) => {
  if (!requireAdmin(context)) return { ok: false as const, kind: "not_authenticated" as const };
  if (!isDbConfigured()) return { ok: false as const, kind: "not_configured" as const };
  const rows = await getDb().select().from(surveys).orderBy(desc(surveys.updatedAt)).limit(100);
  return { ok: true as const, surveys: rows.map(serializeSurvey) };
});

export const getAdminSurveyResults = createServerFn({ method: "GET" }).middleware([adminSessionMiddleware]).validator(z.object({ surveyId: z.string().uuid() })).handler(async ({ context, data }) => {
  if (!requireAdmin(context)) return { ok: false as const, kind: "not_authenticated" as const };
  if (!isDbConfigured()) return { ok: false as const, kind: "not_configured" as const };
  ...
  const responses = await getDb().select({ answers: surveyResponses.answers, locale: surveyResponses.locale }).from(surveyResponses).where(eq(surveyResponses.surveyId, data.surveyId)).orderBy(desc(surveyResponses.createdAt)).limit(10_000);
...
export const createAdminSurvey = createServerFn({ method: "POST" }).middleware([adminSessionMiddleware]).validator(z.object({ slug: z.string().min(2).max(80).regex(/^[a-z0-9-]+$/), title: z.string().min(2).max(160), description: z.string().max(500).optional(), targetLocale: z.string().max(16).optional(), maxResponses: z.number().int().min(1).max(1_000_000).optional(), startsAt: z.coerce.date().optional(), endsAt: z.coerce.date().optional() }).refine((value) => !value.startsAt || !value.endsAt || value.endsAt > value.startsAt, { message: "endsAt must be after startsAt" })).handler(async ({ context, data }) => {
  if (!requireAdmin(context)) return { ok: false as const, kind: "not_authenticated" as const };
  if (!isDbConfigured()) return { ok: false as const, kind: "not_configured" as const };
  const [created] = await getDb().insert(surveys).values(data).returning();
  return { ok: true as const, survey: serializeSurvey(created) };
});

export const createAdminSurveyQuestion = createServerFn({ method: "POST" }).middleware([adminSessionMiddleware]).validator(z.object({ surveyId: z.string().uuid(), type: surveyQuestionTypeSchema, prompt: z.string().min(2).max(2000), options: z.array(z.string().min(1).max(300)).max(100).default([]), config: questionConfigSchema, required: z.boolean().default(false), sortOrder: z.number().int().min(0).max(500).default(0) })).handler(async ({ context, data }) => {
  if (!requireAdmin(context)) return { ok: false as const, kind: "not_authenticated" as const };
  if (!isDbConfigured()) return { ok: false as const, kind: "not_configured" as const };
  if (["single_choice", "multi_choice", "dropdown", "ranking"].includes(data.type) && data.options.length === 0) return { ok: false as const, kind: "invalid_question" as const, message: "This question type requires options." };
  if ((data.type === "matrix_single" || data.type === "matrix_multi") && (!Array.isArray(data.config.rows) || !Array.isArray(data.config.columns) || data.config.rows.length === 0 || data.config.columns.length === 0)) return { ok: false as const, kind: "invalid_question" as const, message: "Matrix questions require rows and columns." };
  const [created] = await getDb().insert(surveyQuestions).values(data).returning();
  return { ok: true as const, question: serializeQuestion(created) };
});

export const setAdminSurveyActive = createServerFn({ method: "POST" }).middleware([adminSessionMiddleware]).validator(z.object({ id: z.string().uuid(), active: z.boolean() })).handler(async ({ context, data }) => {
  if (!requireAdmin(context)) return { ok: false as const, kind: "not_authenticated" as const };
  if (!isDbConfigured()) return { ok: false as const, kind: "not_configured" as const };
...
export const getPublicSurvey = createServerFn({ method: "GET" }).validator(z.object({ slug: z.string().min(2).max(80).regex(/^[a-z0-9-]+$/) })).handler(async ({ data }) => {
  if (!isDbConfigured()) return { ok: false as const, kind: "not_available" as const };
  const now = new Date();
  const [survey] = await getDb().select().from(surveys).where(eq(surveys.slug, data.slug)).limit(1);
  ...
  if (survey.maxResponses) { const [{ count }] = await getDb().select({ count: sql<number>`count(*)::int` }).from(surveyResponses).where(eq(surveyResponses.surveyId, survey.id)); if (count >= survey.maxResponses) return { ok: false as const, kind: "closed" as const }; }
  const questions = await getDb().select().from(surveyQuestions).where(eq(surveyQuestions.surveyId, survey.id)).orderBy(surveyQuestions.sortOrder);
  return { ok: true as const, survey: serializeSurvey(survey), questions: questions.map(serializeQuestion) };
});

export const submitPublicSurvey = createServerFn({ method: "POST" }).validator(z.object({ surveyId: z.string().uuid(), sessionId: z.string().min(16).max(128).optional(), locale: z.string().max(16).optional(), answers: z.record(z.string().uuid(), answerSchema) })).handler(async ({ data }) => {
  if (!isDbConfigured()) return { ok: false as const, kind: "not_available" as const };
  const now = new Date();
