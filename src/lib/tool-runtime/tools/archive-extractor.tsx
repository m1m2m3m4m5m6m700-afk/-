...
function ArchiveExtractorTool() {
  ...
  return (
    <div className="space-y-5 rounded-3xl border border-border bg-card p-6 md:p-8">
      ...
      {busy && <p className="text-sm text-muted-foreground" aria-live="polite">Reading archive…</p>}
      ...
      {entries.length > 0 && <div className="space-y-2" data-testid="extracted-list"><p className="text-xs font-semibold text-muted-foreground">{entries.length} file(s) found</p>{entries.map((entry) => <a key={entry.name} data-testid="extracted-file" href={entry.url} download={entry.name.split("/").pop() || "file"} className="flex items-center justify-between rounded-xl border border-border px-3 py-2 text-sm hover:bg-muted"><span className="truncate">{entry.name}</span><Download className="size-4 shrink-0 text-primary" /></a>)}</div>}
    </div>
  );
}
