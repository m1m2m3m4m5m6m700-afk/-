...
function presetParams(toolId: ToolConfig['id'], preset: Record<string, string | undefined> = {}) {
  ...
  if (toolId === 'image-compressor') return { ...(preset.targetSizeKB ? { targetSizeKB: Number(preset.targetSizeKB) } : {}), ...(preset.format ? { format: preset.format } : {}) };
...
export function planFromWorkflow(workflowId: string, preset: Record<string, string | undefined> = {}): ExecutionPlan | null {
  ...
  const steps = workflow.steps.slice(0, MAX_STEPS).map((step) => ({ toolId: step.toolId, params: { ...(step.params ?? {}), ...(presetParams(step.toolId, preset) ?? {}) } }));
...
function planFromIntent(input: string): ExecutionPlan | null {
  ...
  if (intent.kind === 'tool' && intent.id && EXECUTABLE_PIPELINE_TOOL_ID_SET.has(intent.id)) return { workflowName: 'Direct Tool', confidence: intent.confidence, steps: [{ toolId: intent.id }] };
  return null;
}
