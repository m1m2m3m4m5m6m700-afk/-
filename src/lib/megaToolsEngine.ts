...
function wavBlob(buffer: AudioBuffer): Blob {
  ...
  write(0, "RIFF"); view.setUint32(4, bytes - 8, true); write(8, "WAVE"); write(12, "fmt "); view.setUint32(16, 16, true);
...
async function audioTool(file: File, tool: MegaTool): Promise<MegaToolResult> {
  ...
  const speed = SPEED[tool.preset];
  const length = tool.handler === "trim" ? Math.max(1, Math.floor(buffer.length * (1 - strength))) : tool.handler === "speed" ? Math.max(1, Math.ceil(buffer.length / speed)) : buffer.length;
  const context = new AudioContext();
  try {
    ...
    for (let channel = 0; channel < channels; channel += 1) {
      ...
      for (let index = 0; index < target.length; index += 1) {
        const sourceIndex = tool.handler === "reverse"
          ? Math.max(0, buffer.length - 1 - index)
          : tool.handler === "speed"
            ? Math.min(buffer.length - 1, Math.floor(index * speed))
        ...
        if (tool.handler === "normalize") value *= normalizeScale;
...
async function pdfTool(file: File, tool: MegaTool): Promise<MegaToolResult> {
  ...
  if (tool.handler === "page-numbers") {
    ...
    const originalIndexes = pages.map((_, index) => index);
    const copies = await pdf.copyPages(pdf, originalIndexes);
    copies.forEach((page) => pdf.addPage(page));
    ...
    if (!indexes.length) throw new Error("This PDF has no even-numbered pages to extract.");
    const output = await PDFDocument.create();
    const copied = await output.copyPages(pdf, indexes);
    copied.forEach((page) => output.addPage(page));
    const out = await output.save();
    ...
    if (form.getFields().length === 0) throw new Error("This PDF contains no interactive form fields to flatten.");
    form.flatten();
  } else {
