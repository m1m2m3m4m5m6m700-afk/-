...
async function imageTool(file: File, tool: MegaTool): Promise<MegaToolResult> {
  ...
  try {
    ...
    const filter = filters[tool.handler]; if (filter) context.filter = filter;
    context.drawImage(bitmap, 0, 0, width, height); context.restore();
    let mime: "image/png" | "image/jpeg" | "image/webp" = "image/png"; let extension = "png";
...
async function videoTool(file: File, tool: MegaTool): Promise<MegaToolResult> {
  ...
  try {
    ...
    if (tool.handler === "mute" || tool.handler === "speed") {
      video.muted = tool.handler === "mute"; video.playbackRate = SPEED[tool.preset]; video.controls = true; video.className = "w-full rounded-xl";
      return { type: "video", element: video, cleanup: () => { video.pause(); video.removeAttribute("src"); video.load(); URL.revokeObjectURL(url); } };
    }
    const point = tool.handler === "frame-25" ? 0.25 : tool.handler === "frame-75" ? 0.75 : 0.5;
    if (["poster", "frame-25", "frame-50", "frame-75"].includes(tool.handler)) await seekVideo(video, video.duration * point);
    if (tool.handler === "contact-sheet") {
      ...
      const height = Math.max(1, Math.round(width * ratio)); const canvas = document.createElement("canvas"); canvas.width = width * 2; canvas.height = height * 2;
      const context = canvas.getContext("2d"); if (!context) throw new Error("Canvas is unavailable in this browser.");
      for (const [index, fraction] of [0.2, 0.4, 0.6, 0.8].entries()) { await seekVideo(video, video.duration * fraction); context.drawImage(video, (index % 2) * width, Math.floor(index / 2) * height, width, height); }
      const blob = await new Promise<Blob | null>((resolve) => canvas.toBlob(resolve, "image/jpeg", 0.88)); if (!blob) throw new Error("Could not create the contact sheet.");
      return downloadResult(blob, `${baseName(file.name)}-contact-sheet-${tool.preset}.jpg`);
    }
    let width = video.videoWidth || 1; let height = video.videoHeight || 1;
    ...
    const rotated = tool.handler === "rotate"; const canvas = document.createElement("canvas"); canvas.width = rotated ? height : width; canvas.height = rotated ? width : height;
    const context = canvas.getContext("2d"); if (!context) throw new Error("Canvas is unavailable in this browser."); context.save();
    if (rotated) { context.translate(canvas.width / 2, canvas.height / 2); context.rotate(Math.PI / 2); context.drawImage(video, -width / 2, -height / 2, width, height); }
    else { if (tool.handler === "flip") { context.translate(width, 0); context.scale(-1, 1); } context.drawImage(video, 0, 0, width, height); }
    context.restore(); const blob = await new Promise<Blob | null>((resolve) => canvas.toBlob(resolve, "image/jpeg", 0.88));
    if (!blob) throw new Error("Could not encode the video preview."); return downloadResult(blob, `${baseName(file.name)}-${tool.handler}-${tool.preset}.jpg`);
  } finally { if (tool.handler !== "mute" && tool.handler !== "speed") { video.removeAttribute("src"); video.load(); URL.revokeObjectURL(url); } }
}
...
async function decodeAudio(file: File): Promise<AudioBuffer> {
  ...
  const context = new AudioContext(); try { return await context.decodeAudioData(await file.arrayBuffer()); } finally { await context.close(); }
}
function wavBlob(buffer: AudioBuffer): Blob {
  const channels = buffer.numberOfChannels; const frames = buffer.length; const bytes = 44 + frames * channels * 2; const view = new DataView(new ArrayBuffer(bytes));
  ...
  let offset = 44; for (let frame = 0; frame < frames; frame += 1) for (let channel = 0; channel < channels; channel += 1) { const sample = Math.max(-1, Math.min(1, buffer.getChannelData(channel)[frame] ?? 0)); view.setInt16(offset, sample < 0 ? sample * 0x8000 : sample * 0x7fff, true); offset += 2; }
...
async function audioTool(file: File, tool: MegaTool): Promise<MegaToolResult> {
  ...
  if (tool.handler === "peak" || tool.handler === "rms") {
    let peak = 0; let sum = 0; let count = 0; for (let channel = 0; channel < buffer.numberOfChannels; channel += 1) for (const value of buffer.getChannelData(channel)) { peak = Math.max(peak, Math.abs(value)); sum += value * value; count += 1; }
  ...
  if (tool.handler === "waveform") {
    const canvas = document.createElement("canvas"); canvas.width = 1200; canvas.height = 240; const context = canvas.getContext("2d"); if (!context) throw new Error("Canvas is unavailable in this browser."); const data = buffer.getChannelData(0); const bucket = Math.max(1, Math.floor(data.length / canvas.width)); context.beginPath();
  ...
  const strength = AUDIO_STRENGTH[tool.preset]; const channels = tool.handler === "mono" ? 1 : buffer.numberOfChannels; const length = tool.handler === "trim" ? Math.max(1, Math.floor(buffer.length * (1 - strength))) : buffer.length;
  const context = new AudioContext();
  try {
    const output = context.createBuffer(channels, length, buffer.sampleRate); const left = buffer.getChannelData(0); const right = buffer.numberOfChannels > 1 ? buffer.getChannelData(1) : left;
    for (let channel = 0; channel < channels; channel += 1) { const target = output.getChannelData(channel); for (let index = 0; index < target.length; index += 1) { const sourceIndex = tool.handler === "reverse" ? buffer.length - 1 - index : tool.handler === "speed" ? Math.min(buffer.length - 1, Math.floor(index * SPEED[tool.preset])) : Math.min(buffer.length - 1, index); let value = channel === 0 ? left[sourceIndex] ?? 0 : right[sourceIndex] ?? 0; if (tool.handler === "mono") value = ((left[sourceIndex] ?? 0) + (right[sourceIndex] ?? 0)) / 2; if (tool.handler === "normalize") value *= 0.95; if (tool.handler === "fade-in") value *= Math.min(1, index / Math.max(1, length * strength)); if (tool.handler === "fade-out") value *= Math.min(1, (length - index) / Math.max(1, length * strength)); target[index] = value; } }
...
async function pdfTool(file: File, tool: MegaTool): Promise<MegaToolResult> {
  ...
  if (tool.handler === "extract-text") {
    const pdfjs = await import("pdfjs-dist/legacy/build/pdf.mjs");
    pdfjs.GlobalWorkerOptions.workerSrc = new URL("pdfjs-dist/legacy/build/pdf.worker.mjs", import.meta.url).toString();
    const doc = await pdfjs.getDocument({ data: new Uint8Array(bytes) }).promise; let text = "";
  ...
  if (tool.handler === "page-numbers") { const font = await pdf.embedFont(StandardFonts.Helvetica); pages.forEach((page, index) => page.drawText(String(index + 1), { x: page.getWidth() - 40, y: 20, size: 10, font, color: rgb(0.35, 0.35, 0.35) })); }
  if (tool.handler === "watermark") { const font = await pdf.embedFont(StandardFonts.Helvetica); pages.forEach((page) => page.drawText("Flixo", { x: page.getWidth() / 2 - 15, y: page.getHeight() / 2, size: 18, font, color: rgb(0.75, 0.75, 0.75), opacity: 0.35, rotate: degrees(30) })); }
  if (tool.handler === "remove-metadata") { pdf.setTitle(""); pdf.setAuthor(""); pdf.setSubject(""); pdf.setKeywords([]); pdf.setProducer(""); pdf.setCreator(""); }
  if (tool.handler === "duplicate") { const [copy] = await pdf.copyPages(pdf, [pages.length - 1]); if (copy) pdf.addPage(copy); }
  if (tool.handler === "extract-range") { const start = Math.max(0, Math.floor(pages.length / 2)); const extracted = await PDFDocument.create(); const indices = Array.from({ length: pages.length - start }, (_, offset) => start + offset); const copied = await extracted.copyPages(pdf, indices); copied.forEach((page) => extracted.addPage(page)); const out = await extracted.save(); return downloadResult(blobFromBytes(out, "application/pdf"), `${baseName(file.name)}-extract-range-${tool.preset}.pdf`); }
  if (tool.handler === "split-even") { const extracted = await PDFDocument.create(); const indices = pages.map((_, index) => index).filter((index) => (index + 1) % 2 === 0); if (indices.length === 0) indices.push(0); const copied = await extracted.copyPages(pdf, indices); copied.forEach((page) => extracted.addPage(page)); const out = await extracted.save(); return downloadResult(blobFromBytes(out, "application/pdf"), `${baseName(file.name)}-split-even-${tool.preset}.pdf`); }
  if (tool.handler === "blank-cover") { const [cover] = await pdf.copyPages(pdf, [0]); if (cover) { cover.drawRectangle({ x: 0, y: 0, width: cover.getWidth(), height: cover.getHeight(), color: rgb(1, 1, 1), opacity: 1 }); pdf.insertPage(0, [cover.getWidth(), cover.getHeight()]); } }
  if (tool.handler === "flatten") pages.forEach((page) => page.scaleContent(1, 1));
  if (tool.handler === "poster") { const blob = await renderPdfPoster(bytes); return downloadResult(blob, `${baseName(file.name)}-poster-${tool.preset}.png`); }
