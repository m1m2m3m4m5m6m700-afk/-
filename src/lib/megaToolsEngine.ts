...
export type MegaToolTextResult = {
  type: "text";
  text: string;
};

export type MegaToolDownloadResult = {
  type: "download";
  url: string;
  filename: string;
};

export type MegaToolVideoResult = {
  type: "video";
...
const IMAGE_QUALITY: Record<MegaTool["preset"], number> = {
  ...
  mobile: 0.72, print: 0.90, hd: 0.86, pro: 0.92, max: 0.95,
};

const SPEED: Record<MegaTool["preset"], number> = {
  quick: 0.75, small: 0.85, medium: 0.95, large: 1.10, social: 1.20, web: 1.35,
  mobile: 1.50, print: 0.80, hd: 1.25, pro: 1.50, max: 2.00,
};
...
async function imageTool(file: File, tool: MegaTool): Promise<MegaToolResult> {
  ...
  try {
    ...
    const filters: Partial<Record<MegaTool["handler"], string>> = {
...
async function videoTool(file: File, tool: MegaTool): Promise<MegaToolResult> {
  ...
  try {
    ...
    const position: Record<string, number> = { poster: 0.50, preview: 0.50, "frame-25": 0.25, "frame-50": 0.50, "frame-75": 0.75 };
    if (position[tool.handler] !== undefined) {
      video.currentTime = Math.max(0, Math.min(video.duration || 0, (video.duration || 0) * position[tool.handler]));
    ...
    if (tool.handler !== "mute" && tool.handler !== "speed") URL.revokeObjectURL(url);
  }
}
...
function wavBlob(buffer: AudioBuffer): Blob {
  ...
  const bytes = 44 + frameCount * channels * 2;
...
async function audioTool(file: File, tool: MegaTool): Promise<MegaToolResult> {
  ...
  if (!offline) throw new Error("Offline audio processing is unavailable in this browser.");
  const strength = { quick: 0.15, small: 0.08, medium: 0.05, large: 0.03, social: 0.07, web: 0.04, mobile: 0.10, print: 0.02, hd: 0.025, pro: 0.015, max: 0.01 }[tool.preset];
  const outputChannels = tool.handler === "mono" ? 1 : buffer.numberOfChannels;
  const outputLength = tool.handler === "trim" ? Math.max(1, Math.floor(buffer.length * (1 - strength))) : buffer.length;
  const context = new offline(outputChannels, outputLength, buffer.sampleRate);
  const output = context.createBuffer(outputChannels, outputLength, buffer.sampleRate);
  for (let channel = 0; channel < outputChannels; channel += 1) {
    const target = output.getChannelData(channel);
    const left = buffer.getChannelData(0);
    const right = buffer.numberOfChannels > 1 ? buffer.getChannelData(1) : left;
    for (let index = 0; index < target.length; index += 1) {
      const sourceIndex = tool.handler === "reverse" ? buffer.length - 1 - index : index;
      let value = channel === 0 ? left[Math.min(sourceIndex, left.length - 1)] ?? 0 : right[Math.min(sourceIndex, right.length - 1)] ?? 0;
      if (tool.handler === "mono") value = ((left[Math.min(sourceIndex, left.length - 1)] ?? 0) + (right[Math.min(sourceIndex, right.length - 1)] ?? 0)) / 2;
      if (tool.handler === "normalize") value *= 0.95;
      if (tool.handler === "fade-in") value *= Math.min(1, index / Math.max(1, outputLength * strength));
      if (tool.handler === "fade-out") value *= Math.min(1, (outputLength - index) / Math.max(1, outputLength * strength));
      if (tool.handler === "speed") {
        const ratio = SPEED[tool.preset];
        const accelerated = Math.min(buffer.length - 1, Math.floor(index * ratio));
        value = left[accelerated] ?? 0;
      }
      target[index] = value;
    }
  }
...
async function loadPdf(file: File) {
  ...
  return { PDFDocument, rgb, degrees, pdf };
}

async function pdfTool(file: File, tool: MegaTool): Promise<MegaToolResult> {
  const { PDFDocument, rgb, degrees, pdf } = await loadPdf(file);
  ...
  if (tool.handler === "page-numbers") {
    pages.forEach((page, index) => {
      ...
      page.drawText(String(index + 1), { x: width / 2 - 4, y: 18, size: 10, color: rgb(0.35, 0.35, 0.35) });
    });
  }
  if (tool.handler === "watermark") {
    pages.forEach((page) => {
      const { width, height } = page.getSize();
      page.drawText("FLIXO", { x: width / 2 - 28, y: height / 2, size: 28, rotate: degrees(45), opacity: 0.22, color: rgb(0.25, 0.35, 0.75) });
    });
  }
  if (tool.handler === "duplicate") {
    ...
    const copied = await template.copyPages(pdf, [Math.max(0, pages.length - 1)]);
    if (copied[0]) template.addPage(copied[0]);
    const merged = await template.save();
    return downloadResult(new Blob([merged], { type: "application/pdf" }), `${baseName(file.name)}-duplicate-${tool.preset}.pdf`);
  }
  if (tool.handler === "extract-range" || tool.handler === "split-even") {
    const output = await PDFDocument.create();
    const indices = tool.handler === "split-even" ? pages.map((_, i) => i).filter((i) => i % 2 === 0) : pages.map((_, i) => i).slice(0, Math.max(1, Math.ceil(pages.length / 2)));
    const copied = await output.copyPages(pdf, indices);
    copied.forEach((page) => output.addPage(page));
    const saved = await output.save();
    return downloadResult(new Blob([saved], { type: "application/pdf" }), `${baseName(file.name)}-${tool.handler}-${tool.preset}.pdf`);
  }
  if (tool.handler === "blank-cover") pdf.insertPage(0, [595, 842]);
  if (tool.handler === "flatten") {
    try { pdf.getForm().flatten(); } catch { /* PDFs without forms are already effectively flat. */ }
  }
  if (tool.handler === "poster") {
    ...
    if (!image) throw new Error("Could not render the PDF poster.");
    return downloadResult(image, `${baseName(file.name)}-poster.png`);
  }
