...
const tools = [...manifest.matchAll(/\n    id:\s*"([^"]+)"/g)].map((match) => match[1]);
const contractTools = new Set([...contracts.matchAll(/toolId:\s*"([^"]+)"/g)].map((match) => match[1]));
const requiredChecks = ["render", "interaction", "output", "error", "security", "performance", "mutation", "invariant", "evidence"];

if (tools.length === 0) throw new Error("Certification: public manifest contains no public tools.");
for (const tool of tools) {
  if (!contractTools.has(tool)) throw new Error(`Certification: missing test contract ${tool}`);
}
for (const check of requiredChecks) {
  if (!contracts.includes(`"${check}"`)) throw new Error(`Certification: missing check ${check}`);
}
for (const token of ["ToolCertificationLevel", "ToolCertificationRequirements", 'level: "certified"', "requiredEvidence", "regressionLocked", 'dataProcessing: "local-only"']) {
  if (![types, contracts, manifest, promotion].some((source) => source.includes(token))) {
    throw new Error(`Certification contract missing ${token}`);
  }
}
