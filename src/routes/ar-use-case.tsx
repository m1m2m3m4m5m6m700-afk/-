  ...
  head: ({ params }) => {
    ...
    return {
      ...
      links: [
        ...
        { rel: 'alternate', hrefLang: 'en', href: `${SITE_URL}/en/use-case/${intent.slug}` },
        { rel: 'alternate', hrefLang: 'x-default', href: `${SITE_URL}/en/use-case/${intent.slug}` },
      ],
      ...
      <main dir="rtl" className="image-tool-shell">
        <div className="image-tool-container">
          ...
          <section style={{ maxWidth: 860, paddingTop: 48 }}>
            <p className="image-tool-eyebrow">FLIXO · حل جاهز للنتيجة</p>
            <h1>{copy.title}</h1>
            <p className="image-tool-lead">{copy.description}</p>
            <div className="quickflow-ai-box" style={{ marginTop: 24 }}><div><strong>{copy.outcome}</strong><span>ابدأ بصورة واحدة؛ ويحافظ FLIXO على المسار مختصرًا ويستخدم المعالجة المحلية عندما تدعمها الأدوات.</span></div><Link to={`/ar/quickflow/${intent.workflowId}?preset=${encodeURIComponent(intent.slug)}`} className="primary-button">ابدأ هذا المسار <ArrowRight size={17} /></Link></div>
            <div className="home-workflow-grid" style={{ marginTop: 24 }}>{(workflow?.steps ?? []).map((step, index) => <div key={step.toolId}><span>{String(index + 1).padStart(2, '0')}</span><Check size={18} /><strong>{step.title}</strong><p>{step.optional ? 'خطوة اختيارية عند الحاجة.' : 'خطوة موصى بها لهذه النتيجة.'}</p></div>)}</div>
            <div className="home-principles" style={{ marginTop: 28 }}><div><Zap size={18} /><strong>النتيجة أولًا</strong><span>ابدأ بهدف واضح بدل البحث عن اسم أداة.</span></div><div><Lock size={18} /><strong>محلي أولًا</strong><span>الخطوات المدعومة يمكن تنفيذها داخل المتصفح.</span></div><div><Sparkles size={18} /><strong>قابل للتكرار</strong><span>أعد استخدام المسار دون إعداد كل خطوة من جديد.</span></div></div>
          </section>
        </div>
      </main>
    );
