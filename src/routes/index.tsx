    ...
    return (
      <main className="home-shell">
        ...
        <div className="home-container home-modern">
          ...
          <section className="home-quick-section" aria-labelledby="intent-library-title"><div className="home-section-heading"><div><p className="home-section-kicker">OUTCOME LIBRARY</p><h2 id="intent-library-title">Start from the job you actually need.</h2><p>Each path opens with the right workflow already prepared.</p></div><span>{INTENT_SLUGS_REGISTRY.length} use cases</span></div><div className="home-featured-grid">{INTENT_SLUGS_REGISTRY.map((intentItem) => <Link key={intentItem.slug} to={`/en/use-case/${intentItem.slug}`} className="home-featured-card"><span className="home-featured-icon"><Zap size={20} /></span><span className="home-featured-copy"><strong>{intentItem.title}</strong><span>{intentItem.outcome}</span></span><ArrowUpRight className="home-featured-arrow" size={18} /></Link>)}</div></section>
