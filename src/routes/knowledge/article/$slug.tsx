...
function KnowledgeArticleRoute() {
  ...
  usePageSeo(undefined, {
    title: article ? `${article.title} | Flixo` : "Article Not Found | Flixo",
    description:
      article?.metaDescription || "Learn how Flixo tools fit into modern browser-based workflows.",
    keywords: article?.keywords || ["flixo guide"],
  });

  useEffect(() => {
    trackPageView(`/knowledge/article/${slug}`);
  }, [slug]);
      ...
      <SiteLayout>
        <div className="mx-auto max-w-4xl px-5 py-20 text-center">
          ...
          <p className="mt-2 text-muted-foreground">
            The requested knowledge article does not exist.
          </p>
  ...
  const articleSchema = {
    ...
    author: { "@type": "Organization", name: "Flixo" },
  };
    ...
    <SiteLayout>
      ...
      <div className="bg-hero-glow min-h-screen">
        <article className="mx-auto max-w-4xl px-5 pb-20 pt-10 md:pt-14 space-y-8">
          ...
          <section className="rounded-3xl border border-border/80 bg-card p-6 space-y-5">
