...
function localizedPath(path: string, locale: 'en' | 'ar') { if (path === '/') return locale === 'ar' ? '/ar' : '/'; return locale === 'ar' ? `/ar${path.replace(/^\/en/, '')}` : path; }
function alternateLinks(path: string) { const enPath = localizedPath(path, 'en'); const arPath = localizedPath(path, 'ar'); return `<xhtml:link rel="alternate" hreflang="en" href="${escapeXml(`${SITE_URL}${enPath}`)}"/><xhtml:link rel="alternate" hreflang="ar" href="${escapeXml(`${SITE_URL}${arPath}`)}"/>`; }
...
export const APIRoute = createAPIFileRoute('/sitemap.xml')({
  GET: async () => {
    ...
    const urls = basePaths.flatMap((item) => [{ ...item, path: localizedPath(item.path, 'en') }, { ...item, path: localizedPath(item.path, 'ar') }]);
