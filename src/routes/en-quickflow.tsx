...
export const enQuickFlowRoute = createRoute({
  ...
  component: function QuickFlowPage() {
    ...
    const reset = () => {
      ...
      setResult(null);
      setProgress(null);
      setPlan(planFromWorkflow(workflow.id));
      setError('');
    ...
    return (
      <main className="image-tool-shell">
        <div className="image-tool-container quickflow-shell">
          ...
          <p className="image-tool-eyebrow" style={{ marginTop: 28 }}>QUICKFLOW · LOCAL PIPELINE</p>
          ...
          <section className="compressor-card quickflow-run-card" aria-label="Run QuickFlow">
            <label className="upload-zone" htmlFor="quickflow-file">
              ...
              <span className="upload-title">{file ? file.name : 'Choose an image to run this flow'}</span>
              <span className="upload-subtitle">Processing stays in your browser after the plan is chosen.</span>
            </label>
            ...
            <div className="quickflow-ai-box">
              <div><strong>Need a different plan?</strong><span>Ask FLIXO AI for a safe execution chain; the browser still runs every image step.</span></div>
              <div className="quickflow-ai-row"><input value={aiPrompt} onChange={(event) => setAiPrompt(event.target.value)} placeholder="e.g. make this ready for an online store" /><button type="button" className="secondary-button" disabled={aiBusy || !aiPrompt.trim()} onClick={generateAiPlan}>{aiBusy ? 'Planning…' : 'Plan with AI'}</button></div>
            </div>
            ...
            <section className="quickflow-card" aria-label={`${workflow.title} execution steps`}>
              {plan?.steps.map((step, index) => <div key={`${step.toolId}-${index}`} className={`quickflow-step ${progress?.currentToolId === step.toolId ? 'is-next' : ''} ${progress && progress.currentStepIndex > index + 0 ? 'is-done' : ''}`}>
                <div className="quickflow-step-number">{progress && progress.currentStepIndex > index ? <CheckCircle2 size={20} /> : <span>{index + 1}</span>}</div>
                <div className="quickflow-step-copy"><strong>{step.toolId}</strong><span>{JSON.stringify(step.params ?? {})}</span></div>
              </div>)}
            </section>

            {error && <div className="error-box" role="alert">{error}</div>}
            <div className="quickflow-footer-actions">
              <button type="button" className="primary-button" disabled={busy || !file} onClick={run}>{busy ? <><Zap size={16} /> Running…</> : <><Zap size={16} /> Run workflow</>}</button>
              <button type="button" className="secondary-button" disabled={!file && !result} onClick={reset}><RotateCcw size={16} /> Repeat</button>
            </div>
          </section>

          {result && <section className="result-card quickflow-result"><h2>Result ready</h2><p>All enabled steps completed locally in the browser.</p><img src={resultUrl} alt="FLIXO QuickFlow result" className="preview-image" /><div className="quickflow-footer-actions"><a className="download-button" href={resultUrl} download={`flixo-${workflow.id}.webp`} onClick={() => trackProductEvent('download_clicked', { workflowId: workflow.id })}><Download size={16} /> Download result</a><Link className="secondary-button" to="/">Back to goals <ArrowRight size={16} /></Link></div></section>}
        </div>
      </main>
    );
  },
