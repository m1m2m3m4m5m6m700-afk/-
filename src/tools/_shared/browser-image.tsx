...
export function BrowserImageTool({ mode, title, accept = 'image/*', multi = false }: Props) {
  ...
  const locale = getToolUiLocale(location.pathname) as ToolUiLocale;
  const t = TOOL_UI_DICTIONARY[locale];
  const isArabic = locale === 'ar';
  const localizedTitle = t.titles[mode] ?? title;
  ...
  async function run() {
    ...
    try {
      ...
      else if (mode === 'image-effects') { ctx.filter = `brightness(${effect.brightness}%) contrast(${effect.contrast}%) saturate(${effect.saturate}%) grayscale(${effect.grayscale}%)`; ctx.drawImage(image, 0, 0, width, height); ctx.filter = 'none'; }
      else if (mode === 'exif-cleaner') { ctx.drawImage(image, 0, 0, width, height); }
  ...
  return <main dir={isArabic ? 'rtl' : 'ltr'} lang={isArabic ? 'ar' : 'en'} className="mx-auto max-w-3xl px-6 py-10">
    <h1 className="text-3xl font-bold">{localizedTitle}</h1>
    <p className="mt-2 text-sm opacity-70">{t.clientSide}</p>
    <label className="mt-6 flex min-h-28 cursor-pointer items-center justify-center rounded-xl border border-dashed p-6 text-center" htmlFor={`${mode}-file`}><span>{t.dragDrop}</span></label>
    <input id={`${mode}-file`} className="sr-only" type="file" accept={accept} multiple={multi} onChange={(event) => setFiles(Array.from(event.target.files ?? []))} />
    {mode === 'watermark-adder' && <input aria-label={t.watermarkText} className="mt-4 w-full rounded border p-2" value={text} onChange={(e) => setText(e.target.value)} placeholder={t.watermarkText} />}
    {mode === 'meme-generator' && <div className="mt-4 grid gap-2"><input aria-label={t.topText} className="rounded border p-2" value={top} onChange={(e) => setTop(e.target.value)} placeholder={t.topText} /><input aria-label={t.bottomText} className="rounded border p-2" value={bottom} onChange={(e) => setBottom(e.target.value)} placeholder={t.bottomText} /></div>}
    {mode === 'image-effects' && <div className="mt-4 grid gap-2 sm:grid-cols-2"><label>{t.brightness} <input aria-label={t.brightness} type="range" min="50" max="150" value={effect.brightness} onChange={(e) => setEffect({ ...effect, brightness: Number(e.target.value) })} /></label><label>{t.contrast} <input aria-label={t.contrast} type="range" min="50" max="150" value={effect.contrast} onChange={(e) => setEffect({ ...effect, contrast: Number(e.target.value) })} /></label><label>{t.saturation} <input aria-label={t.saturation} type="range" min="0" max="200" value={effect.saturate} onChange={(e) => setEffect({ ...effect, saturate: Number(e.target.value) })} /></label><label>{t.grayscale} <input aria-label={t.grayscale} type="range" min="0" max="100" value={effect.grayscale} onChange={(e) => setEffect({ ...effect, grayscale: Number(e.target.value) })} /></label></div>}
    <button className="mt-6 rounded bg-black px-5 py-3 text-white" type="button" disabled={busy} onClick={run}>{busy ? t.processing : t.runTool}</button>
    {error && <p role="alert" className="mt-4 text-red-600">{error}</p>}
    {result && <section className="mt-8 rounded-xl border p-4" aria-live="polite"><div className="mb-3 font-semibold">{t.result}</div>{result.text ? <pre className="max-h-72 overflow-auto text-xs">{result.text}</pre> : <img className="max-h-[28rem] w-full object-contain" src={result.url} alt={localizedTitle} />}{!result.text && <p className="mt-2 text-sm opacity-70">{status}</p>}<button className="mt-4 rounded border px-4 py-2" type="button" onClick={() => download(result)}>{t.downloadNow}</button></section>}
  </main>;
}
