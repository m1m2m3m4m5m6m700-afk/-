...
export function ImageCompressor() {
  ...
  const compressImage = useCallback((srcUrl: string, targetQuality: number, targetFormat: "jpeg" | "webp" | "png") => {
    const img = new Image();
    ...
    img.onload = () => {
      try {
        ...
        }, `image/${targetFormat}`, targetQuality / 100);
      } catch (err) {
        console.error(err);
  ...
  const handleFileSelect = (selectedFile: File) => {
    ...
    setOriginalUrl(url);
    const defaultFmt: "jpeg" | "webp" | "png" = selectedFile.type.includes("png") ? "png" : selectedFile.type.includes("webp") ? "webp" : "jpeg";
  ...
  return (
    <div className="rounded-3xl border border-border bg-card/80 p-4 shadow-soft backdrop-blur md:p-6">
      {!file ? (
        <div onDrop={(e) => { e.preventDefault(); setIsDragging(false); const dropped = e.dataTransfer.files?.[0]; if (dropped) handleFileSelect(dropped); }} onDragOver={(e) => { e.preventDefault(); setIsDragging(true); }} onDragLeave={(e) => { e.preventDefault(); setIsDragging(false); }} onClick={() => fileInputRef.current?.click()} className={cn("flex cursor-pointer flex-col items-center justify-center rounded-2xl border-2 border-dashed p-10 text-center transition-all duration-300 min-h-72", isDragging ? "border-primary bg-primary/10 scale-[1.01]" : "border-border/80 bg-background/50 hover:border-primary/50 hover:bg-card/90")}>
          <input ref={fileInputRef} type="file" accept="image/png, image/jpeg, image/webp" className="hidden" onChange={(e) => { const selected = e.target.files?.[0]; if (selected) handleFileSelect(selected); }} />
        ...
        <div className="space-y-6">
          ...
          <div className="rounded-2xl border border-border/60 bg-surface/40 p-4 space-y-4">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
              <div className="flex-1"><div className="flex items-center justify-between text-xs mb-1.5"><span className="font-semibold text-foreground flex items-center gap-1.5"><Sliders className="size-3.5 text-primary" />{t("imageCompressor.quality", { count: quality })}</span><span className="text-muted-foreground text-[11px]">{quality > 85 ? t("imageCompressor.qualityHigh") : quality > 50 ? t("imageCompressor.qualityBalanced") : t("imageCompressor.qualityMax")}</span></div><Slider value={[quality]} min={5} max={95} step={1} onValueChange={(val) => { const next = val[0] ?? quality; setQuality(next); if (originalUrl) compressImage(originalUrl, next, format); }} /></div>
              <div className="w-full sm:w-44"><span className="text-xs font-semibold text-foreground block mb-1.5">{t("imageCompressor.format")}</span><Select value={format} onValueChange={(value: "jpeg" | "webp" | "png") => { setFormat(value); if (originalUrl) compressImage(originalUrl, quality, value); }}><SelectTrigger className="h-9 text-xs rounded-xl"><SelectValue /></SelectTrigger><SelectContent><SelectItem value="jpeg">JPEG (.jpg)</SelectItem><SelectItem value="webp">WebP (.webp)</SelectItem><SelectItem value="png">PNG (.png)</SelectItem></SelectContent></Select></div>
            </div>
          </div>

          <div className="relative min-h-72 flex flex-col items-center justify-center rounded-2xl border border-border bg-background/50 p-4">{compressedUrl && <img src={compressedUrl} alt={t("imageCompressor.compressedPreview")} className="max-h-80 object-contain rounded-xl shadow-md animate-rise" />}</div>

          <div className="flex items-center justify-between gap-3 border-t border-border/60 pt-4"><Button variant="outline" size="sm" onClick={handleReset} className="rounded-xl text-xs"><RotateCcw className="me-1.5 size-3.5" />{t("imageCompressor.compressAnother")}</Button><Button onClick={handleDownload} disabled={!compressedUrl} size="sm" className="rounded-xl shadow-xs text-xs"><Download className="me-1.5 size-3.5" />{t("imageCompressor.download")}</Button></div>
        </div>
      )}
