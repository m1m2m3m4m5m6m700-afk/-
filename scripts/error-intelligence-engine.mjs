...
function detectRootCause(report) {
  ...
  const patterns = [
    ...
    { re: /build-rate-limit|api-deployments-free-per-day/i, code: "deployment-rate-limit", cause: "The external deployment provider blocked another deployment because of a rate limit.", fix: "Wait for the provider limit reset or use an approved deployment capacity; do not retry repeatedly." },
  ...
  return {
    code: "unclassified",
    rootCause: "No deterministic root-cause rule matched the normalized failure payload.",
    recommendation: "Inspect the first failing command and stack trace, then add a regression rule after the diagnosis is confirmed.",
    confidence: 0.35,
  };
}
...
current.classification = {
  deterministic: diagnosis.code !== "unclassified",
...
const signatureInput = JSON.stringify({
  ...
  message: current.message.slice(0, 2000),
});
current.fingerprint = hash(signatureInput).slice(0, 32);

const priorMemory = (await safeReadJson(`${root}/${memoryPath}`)) ?? { schemaVersion: 1, entries: {} };
...
current.memory = {
  lookup: "deterministic-file-memory",
  hit: Boolean(previous),
  occurrences: priorMemory.entries[key].occurrences,
  previousDiagnosis: previous?.rootCause ?? null,
};
