...
if (!fs.existsSync(reportPath)) { console.error(`Certification report missing: ${reportPath}`); process.exit(1); }
const report = JSON.parse(fs.readFileSync(reportPath, "utf8"));
if (report.verdict !== "CERTIFIED") { console.log(`Baseline not generated because verdict is ${report.verdict}.`); process.exit(0); }
...
const baselineId = `${slug}-baseline-v${version}`;
const baseline = {
  schemaVersion: 1,
  baselineId,
  version,
  ...
  performance: { ...(report.performance || {}), mediumGateMs: report.performance?.mediumGateMs ?? null },
  evidence: { hashes: report.evidence?.hashes ?? {}, baselineId }
};
fs.writeFileSync(path.join(baselineDir, "certification-baseline.json"), JSON.stringify(baseline, null, 2) + "\n");
console.log(`Baseline generated: ${path.join(baselineDir, "certification-baseline.json")}`);
