...
test('strategic planner creates a guarded TypeScript remediation step', () => {
  const plan = buildStrategicPlan({
    diagnosis: {
      known: true,
      knownPattern: 'typescript',
      layer: 'TYPECHECK',
    },
  });

  assert.equal(plan.status, 'planned');
  assert.equal(plan.steps[0].id, 'typecheck-repair-review');
  assert.equal(plan.steps[0].gate, 'Fast');
  assert.equal(plan.steps[0].autoApply, false);
  assert.deepEqual(plan.steps[0].validation, ['npm run typecheck', 'affected tool tests']);
});
