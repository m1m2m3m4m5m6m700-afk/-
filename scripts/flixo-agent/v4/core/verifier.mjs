...
export function verifyHypothesis(hypothesis, { branch = null, apply = false } = {}) {
  ...
  if (apply && branch !== DEVELOPMENT_BRANCH) errors.push(`v4 may write only to development branch: ${DEVELOPMENT_BRANCH}`);
  if (branch === 'main') errors.push('main is protected and never writable by v4');
  ...
  return { valid: errors.length === 0, errors, allowedBranch: DEVELOPMENT_BRANCH, applyAllowed: apply === true && branch === DEVELOPMENT_BRANCH };
}
...
export function verifyHypothesisSet(hypotheses, options = {}) {
  ...
  return { valid: results.every(r => r.valid), results, errors: results.flatMap(r => r.errors.map(e => `${r.experimentId}: ${e}`)), allowedBranch: DEVELOPMENT_BRANCH, applyAllowed: options.apply === true && options.branch === DEVELOPMENT_BRANCH };
}
