...
export function verifyHypothesis(hypothesis, { branch = null, apply = false } = {}) {
  ...
  if (apply && branch !== DEVELOPMENT_BRANCH) errors.push('production promotion must be handled outside v4');
  ...
  return { valid: errors.length === 0, errors, allowedBranch: DEVELOPMENT_BRANCH };
}
...
export function verifyHypothesisSet(hypotheses, options = {}) {
  ...
  return { valid: results.every(r => r.valid), results, errors: results.flatMap(r => r.errors.map(e => `${r.experimentId}: ${e}`)), allowedBranch: DEVELOPMENT_BRANCH };
}
