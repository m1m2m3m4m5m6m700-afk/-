...
const cases = [
  ...
  { id: "near-capacity-unicode", options: { mode: "text", input: nearCapacityUnicode }, expected: nearCapacityUnicode, width: 1000, errorCorrectionLevel: "L" },
...
for (const testCase of cases) {
  ...
  let pngDataUrl;
  try {
    pngDataUrl = await QRCode.toDataURL(payload, options);
  } catch (error) {
    throw new Error(`${testCase.id}: QR generation failed: ${error instanceof Error ? error.message : String(error)}`);
  }
  ...
  assert.ok(decoded, `${testCase.id}: independent PNG decode returned no QR result (payload bytes=${Buffer.byteLength(testCase.expected, "utf8")}, width=${width}, ec=${errorCorrectionLevel})`);
  ...
  results.push({ id: testCase.id, status: "PASS", generated: true, pngBytes: pngBuffer.length, pngWidth: image.width, pngHeight: image.height, decoded: decoded.data, width, errorCorrectionLevel, payloadBytes: Buffer.byteLength(testCase.expected, "utf8") });
}
