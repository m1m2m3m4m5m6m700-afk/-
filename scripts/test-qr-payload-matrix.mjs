...
const cases = [
  ...
  { id: "long-unicode", options: { mode: "text", input: "مرحبا Flixo — QR ✓ اختبار ".repeat(40) }, expected: "مرحبا Flixo — QR ✓ اختبار ".repeat(40) },
...
for (const testCase of cases) {
  ...
  const options = { width: 300, margin: 2, errorCorrectionLevel: "M", ...(testCase.color ? { color: testCase.color } : {}) };
  ...
  const svg = await QRCode.toString(payload, { type: "svg", width: 300, margin: 2, errorCorrectionLevel: "M", ...(testCase.color ? { color: testCase.color } : {}) });
  ...
  results.push({ id: testCase.id, status: "PASS", generated: true, pngBytes: pngBuffer.length, pngWidth: image.width, pngHeight: image.height, decoded: decoded?.data });
}
